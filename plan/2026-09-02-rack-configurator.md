...
## Epic 0a — Dust filter engraving (CAD)
...
- [ ] `0a.1` Delete `components/textover.scad` (dead, references removed preset fields).
- [ ] `0a.2` Add `dust_label_top` / `dust_label_bottom` string params, default empty.
- [ ] `0a.3` Engrave both bands on the front face, centred, depth `textover_depth`.
- [ ] `0a.4` Assert the rendered text stays inside the 8.6 mm band.
- [ ] `0a.5` `scripts/test-dust-label.sh` — renders with and without labels, checks bbox unchanged.
- [ ] `0a.6` Decide web delivery: free-form text cannot be pre-baked. Ship blank in v1.

## Epic 0b — Gyroid rework (CAD)

- [ ] `0b.1` Spike the fold: four 90° bands via the verified symmetries. Confirms the loop is real.
- [ ] `0b.2` Generate the gyroid asset offline (numpy + marching cubes), commit to `cad/assets/`.
- [ ] `0b.3` Replace `gyroidCutter()` with import + intersect; retire the phase assert.
- [ ] `0b.4` Single knob `face_vent_gyroid_period`; derive `face_thickness` and `face_depth`.
- [ ] `0b.5` Propagate the depth change: stack screw length, dust filter fit, lip interfaces.
- [ ] `0b.6` Extend `scripts/test-vent-patterns.sh` to assert a triangle-count ceiling per pattern.

## Epic 1 — Pipeline (blocks the website)

- [ ] `1.1` Add the vent-pattern axis to `generate-stl.sh`. Back Face falls back to `triangles`
